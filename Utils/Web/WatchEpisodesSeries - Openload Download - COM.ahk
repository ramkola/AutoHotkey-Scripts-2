...
win_title = .*Season\s\d+\sEpisode\s\d+\s[s|S]\d+[e|E]\d+\sWatch\sOnline.*Google Chrome ahk_class Chrome_WidgetWin_1 ahk_exe chrome.exe
...
re_needle = http://www.watchepisodeseries.com/(.*?)-season-(\d+)-episode-(\d+)-[s|S]\d+[e|E]\d+_\d+ 
show_name := RegExReplace(watchepisodeseries_url, re_needle, "$1")
StringUpper, show_name_capitalized, show_name, T
; 
season_num := RegExReplace(watchepisodeseries_url, re_needle, "$2")
next_episode_num := RegExReplace(watchepisodeseries_url, re_needle, "$3")
next_episode_title := show_name_capitalized " Season " season_num " Episode " next_episode_num + 1
new_season_title := show_name_capitalized " Season " season_num + 1 " Episode 1"
...
links_list := []
links_list := get_links_list(wb, watchepisodeseries_url)
...
{
    OutputDebug, % "--------------------------------------------------------------------------"
    OutputDebug, % Format("link_list #{:02}) {}", A_Index, trim(links_list[A_Index]))
    openload_link := ""
    html_source := ""
...
get_links_list(p_wb,p_url)
{
    p_links_list := []
    html_source := p_wb.document.documentElement.outerHTML
    tag_regex_url := RegExReplace(p_url,"^(.*?)_\d+$","$1") 
    tag_regex_url = <a href="(%tag_regex_url%.*?)               
    ;"
    re_needle1 := tag_regex_url Chr(34) ">openload\.[c|i]o</a>"
